...
def data_select():
    ...
    TabuList = []  # 筛选出局的股票代码
    data = pd.read_csv('E:\\CODE\Python\Deecamp\Proj\Data\instruments_ansi.csv')
    ...
    for i in range(0, len(org_data_list)):
        ...
        for key in data_list.keys():
            if key in fund_info.keys():
                if key == 'Data_assetAllocation' and 'Data_assetAllocationCurrency' in fund_info.keys():
...
def solve_manager_info():
    ...
    df_manager_info_list.to_csv('Data/manager.csv')
